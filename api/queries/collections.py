...
class CollectionQueries(Queries):
    ...
    def add_card_to_collection(self, card: dict, account_id: str) -> CollectionOut:
        ...
        self.collection.find_one_and_update({"account_id": ObjectId(account_id)}, {"$set": collection}, return_document=ReturnDocument.AFTER)

        collection["account_id"] = str(collection["account_id"])
        collection["id"] = str(collection["_id"])
        return collection

    def remove_one_card_copy_from_collection(self, multiverse_id: int, account_id: str) -> CollectionOut:
        collection = self.collection.find_one({"account_id": ObjectId(account_id)})
        card_list = collection.get("cards")
    ...
    def remove_all_card_copies_from_collection(self, multiverse_id: int, account_id: str) -> CollectionOut:
        collection = self.collection.find_one({"account_id": ObjectId(account_id)})
        card_list = collection.get("cards")
