...
function AddToDeckForm(props) {
  ...
  return (
    <div className="card mb-4 p-4 box-shadow">
      <Form method="put" onSubmit={handleSubmit} name="deck">
        <Form.Select
          className="mb-2"
        ...
        >
          ...
          {decksData.decks.map((selectedDeck) => {
            return (
              <option
                key={`${selectedDeck.name} ${selectedDeck.id}`}
        ...
        decksData.decks
          ...
          .includes(multiverseId) ? (
          <Button className="mb-2" variant="primary" size="sm" type="submit">
            + Add more to deck (
            {
              decksData.decks
                .find((deck) => deck.id === deckId)
                .cards.find((card) => card.multiverse_id === multiverseId)
                .quantity
            }
            )
          </Button>
      ...
      <Form method="put" onSubmit={handleSubmit} name="collection">
        {collectionData.cards
          .map((card) => card.multiverse_id)
          .includes(multiverseId) ? (
          <Button variant="primary" size="sm" type="submit">
            + Add more to collection (
            {
              collectionData.cards.find(
                (card) => card.multiverse_id === multiverseId
              ).quantity
            }
            )
          </Button>
